    ...
    loadData(`https://api.github.com/users/jmsmarcelo/repos?per_page=${repoPerPage}&page=${repoPage}&sort=created&direction=desc`)
        .then(json => {
            ...
            json.forEach(repo => {
                ...
                if(repo.language) elems[repo.name + '_lang'] = [repo.name + '_info', 'span', [['innerHTML', repo.language || '&nbsp;'], ['classList', `repo-lang ${repo.language.toLowerCase().replace(/\+/g, 'p')}`]]];
                if(repo.stargazers_count > 0) elems[repo.name + '_star'] = [repo.name + '_info', 'span', [['innerHTML', repo.stargazers_count || '&nbsp;'], ['classList', 'repo-icons star']]];
            });
